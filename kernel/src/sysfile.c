...
uint64 sys_link(void)
{
	...
	if(dp->dev != ip->dev || ip->inode_ops->dir_link(dp, name, ip->inum) < 0) {
...
uint64 sys_unlink(void)
{
	...
	// Cannot unlink "." or "..".
	if(root_fs->vfs_ops->name_cmp(name, ".") == 0 || root_fs->vfs_ops->name_cmp(name, "..") == 0)
		goto bad;

	if((ip = dp->inode_ops->dir_lookup(dp, name, &off)) == 0)
		goto bad;
...
static struct inode * create(char * path, short type, short major, short minor)
{
	...
	if(type == T_DIR) {  // Create . and .. entries.
		...
		if(ip->inode_ops->dir_link(ip, ".", ip->inum) < 0 || ip->inode_ops->dir_link(ip, "..", dp->inum) < 0)
			panic("create dots");
	}

	if(dp->inode_ops->dir_link(dp, name, ip->inum) < 0)
